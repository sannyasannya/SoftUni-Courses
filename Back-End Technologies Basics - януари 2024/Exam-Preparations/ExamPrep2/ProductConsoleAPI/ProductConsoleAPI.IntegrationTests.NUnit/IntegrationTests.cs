...
namespace ProductConsoleAPI.IntegrationTests.NUnit
{
    public  class IntegrationTests
    {
        ...
        public async Task GetAllAsync_WhenProductsExist_ShouldReturnAllProducts()
        {
            ...
            var secondItem = result.FirstOrDefault(x => x.ProductCode == secondProduct.ProductCode);
        ...
        [Test]
        public async Task SearchByOriginCountryAsync_WithInvalidOriginCountry_ShouldThrowArgumentException()
        {
            // Act & Assert
        ...
        public async Task UpdateAsync_WithValidProduct_ShouldUpdateProduct()
        {
            ...
            // Assert
            var productInDb = await productsManager.GetSpecificAsync(firstProduct.ProductCode);
            Assert.IsNotNull(productInDb);

            Assert.That(productInDb.ProductName, Is.EqualTo(firstProduct.ProductName));
            Assert.That(productInDb.Description, Is.EqualTo(firstProduct.Description));
            Assert.That(productInDb.Price, Is.EqualTo(firstProduct.Price));
            Assert.That(productInDb.Quantity, Is.EqualTo(firstProduct.Quantity));
            Assert.That(productInDb.OriginCountry, Is.EqualTo(firstProduct.OriginCountry));
            Assert.That(productInDb.ProductCode, Is.EqualTo(firstProduct.ProductCode));
        }
